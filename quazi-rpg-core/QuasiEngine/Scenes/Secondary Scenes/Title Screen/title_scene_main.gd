...
@onready var load_menu = preload("res://QuasiEngine/Scenes/Save System/save_menu.tscn")
...
func _create_menu() -> Node:
	...
	file_menu.load_save_menu.connect(_on_switch_to_save_menu)
	return file_menu
...
func _on_switch_to_save_menu(file_menu_mode: FileManagerMenu.FileMenuMode) -> void:
	_kill_autoload_objects()
	_kill_filescreen_objects()
	_create_load_menu(file_menu_mode)
...
func _create_load_menu(file_menu_mode) -> void:
	var _load_menu = load_menu.instantiate()
	file_screen_stage.add_child(_load_menu)
	_load_menu.load_mode()
