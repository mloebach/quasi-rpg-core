...
func _create_global_save(dir: DirAccess):
	var global_data = FileAccess.open(GlobalData.global_save_path, FileAccess.WRITE)
	var new_data = GlobalSave.new()
	#var new_settings = Settings.settings_options
	Settings.set_values_to_default()
	global_data.store_line(JSON.stringify(new_data.main_to_json().data))
	var setting_data = FileAccess.open(GlobalData.settings_path, FileAccess.WRITE)
	setting_data.store_line(JSON.stringify(Util.to_json(Settings.settings_options.to_dict()).data))
	#print("global data json" + JSON.stringify(new_data.main_to_json().data))
	#print("Current Volume: " + new_d)
	global_data.close()
